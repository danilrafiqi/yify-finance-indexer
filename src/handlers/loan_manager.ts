...
ponder.on("YIFYLoanManagerV2:NFTDeposited", async ({ event, context }) => {
    try {
        ...
        // Check if loan already exists to avoid primary key conflict
        ...
        console.error("Error in NFTDeposited handler:", error);
        throw error;
    }
});
...
ponder.on("YIFYLoanManagerV2:Borrowed", async ({ event, context }) => {
    try {
        ...
        console.error("Error in Borrowed handler:", error);
        throw error;
    }
});
...
ponder.on("YIFYLoanManagerV2:Repaid", async ({ event, context }) => {
    try {
        ...
        console.error("Error in Repaid handler:", error);
        throw error;
    }
});
...
ponder.on("YIFYLoanManagerV2:NFTWithdrawn", async ({ event, context }) => {
    try {
        ...
        console.error("Error in NFTWithdrawn handler:", error);
        throw error;
    }
});
